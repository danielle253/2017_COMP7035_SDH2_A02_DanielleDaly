...
public class MyMain {
	...
	public static void test(){
		...
		resM = ex.smallerMyList(m2, 9); // 3 , 2
		ex.recursiveDisplayElements(resM);

		resM = ex.smallerMyList(m3, 6); // 4, 1, 3, 5, 2
		ex.recursiveDisplayElements(resM);
		...
		resM = ex.biggerEqualMyList(m2, 9); // 9
		ex.recursiveDisplayElements(resM);

		resM = ex.biggerEqualMyList(m3, 6); // 8, 6, 6
		ex.recursiveDisplayElements(resM);
		...
		resM = ex.concatenate(m1, m2); // 9, 3, 2
		ex.recursiveDisplayElements(resM);

		resM = ex.concatenate(m2, m1); // 9, 3, 2
		ex.recursiveDisplayElements(resM);		

		resM = ex.concatenate(m2, m3); // 9, 3, 2, 4, 8, 1, 3, 6, 5, 2, 6 
		ex.recursiveDisplayElements(resM);
		...
		resM = ex.quickSort(m2); // 2 3 9 
		ex.recursiveDisplayElements(resM);

		resM = ex.quickSort(m3); // 1 2 2 3 3 4 5 6 6 8 9
		ex.recursiveDisplayElements(resM);
